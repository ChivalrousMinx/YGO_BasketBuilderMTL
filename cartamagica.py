...
def url_request(site, input):
    ...
    elif site == 'Cartamagica':
        url = 'https://www.cartamagica.com/products/search?q='+input
    else:
        return("error")
...
def search_cards(clean_liste,titre):
    ...
    for i in clean_liste:
        ...
        if name:
            if re.search(titre.lower(),i.lower()):
                #print(titre.lower())
                #print(i.lower())
                #print("regarde icitte too : it worked!!!")
                t.append(cn)
            cn+=1
    return(t)
...
def calcul_prix(clean_liste,t):
    ...
    for i in range(0,len(clean_liste)):
        ...
        noprice=re.search("<span class=.variant-short-info variant-qty.>Out of stock.</span>",clean_liste[i-2]) and re.search("<span class=.price no-stock hide. itemprop=.price.>",clean_liste[i-1])
        if price or noprice:
            if cp in t:
                if price:
...
def main():
    nom=''
    nom=input("Entrer le nom de la carte recherchée. \n(Les majuscules et les espaces sont importants)\n=>")

    input2=urllib.parse.quote_plus(nom)
    liste=nom.split(" ")
    titre=" ".join(liste)

    cl=clean_liste(url_request('Altf4',input2))
    #print(calcul_prix(cl,search_cards(cl,titre)))
...
def DecklistCM(path):
    ...
    for l in lines:
        ...
        time.sleep(1)
        cl=clean_liste(url_request('Cartamagica',input2))
...
def DecklistOnlyCM(path):
    ...
    print("------tableau cartamagica loaded------")
